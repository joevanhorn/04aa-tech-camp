...
| T5 | **Org provisioning: example agent** | see T8 | The live successor to `archive/provision_lab_org.py:456-473` is the managed-assets baseline servicer; both example-agent changes (signOnProvider create + async-202 handling) are specified in T8. Service-app admin role requirement is already documented at `lab-infra/README.md:92-98`, no change there. |
...
| T8 | **managed-assets baseline servicer (initial org state)** | `managed-assets-mirror` `src/servicers/labs/techcamp-o4aa/TechCampO4AALabServicer.js:375-403`; `src/external/wic/index.js:897-911` | Reviewed 2026-08-17. Two changes: (a) **Latent async bug**: `createAiAgent()` is a bare `postJson` with no 202 handling. On migrated orgs, register is asynchronous and can return an empty body, so `agent?.id` comes back undefined and the CRM managed-connection step is silently skipped, leaving the pre-loaded `VantageCRM Example Agent` without the connection that Module 1 §1.8 and Module 2 §2.9 tour. Fix: on empty create response, re-fetch by `profile.name` (or poll the operations API) before creating the connection. (b) **Model fidelity**: the example agent is created bare (`profile` only), so on a migrated org it has no `signOnProvider` binding, while every agent attendees register in Module 2 gets an auto-created app. Create it with `signOnProvider: {type: 'NEW_OIDC_APP'}` so the toured example matches what attendees build (agent stays STAGED; its app stays INACTIVE, which is fine). Teardown is safe as-is: agent DELETE (202) cascade-deletes the auto-created app, and the servicer only deletes apps it created itself. No changes needed to the AS policy config (`config.js:233` already grants token-exchange + jwt-bearer) or to the Toolkit / Admin UI app definitions (still ordinary `0oa` apps). NOTE: the mirror repo is a copy; land these via the real managed-assets pipeline, not the mirror's main. |
| T9 | **Full E2E validation run before content freeze** | xaa2 org + new bundle | Re-run the Module 2-5 flow end to end (register with auto-create, wire CRM, tool calls as all three personas, OIG request/cert, kill switch) on a migrated org with the new bundle. The xaa2 validation covered the adapter chain; this run covers the lab's exact scripts and personas. |
...
| Skip | Persona/group/Toolkit-only shots | Module 1 L52-187, Module 3 Toolkit shots, Module 5 OIG flow shots | Unaffected by the platform change unless the E2E run (T9) shows otherwise. OIG and Toolkit UIs did not move. |

## Sequencing

1. T1 (PR merge + bundle) and T2/T3 (infra scripts) first; T8 (baseline servicer) lands via the managed-assets pipeline in parallel; together they gate T9.
2. T9 E2E validation run on a migrated org locks the exact UI labels and token values.
3. M-items written against T9's observed reality (especially M2's tab names).
