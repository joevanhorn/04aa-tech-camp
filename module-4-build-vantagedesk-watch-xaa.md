...
Build the entire VantageDesk side in your org (authorization server, scopes, access policy, managed connection, and the matching MCP Adapter resource) using your VantageCRM work as the template. Then watch the XAA protocol carry both agent and user identity all the way to the app.

**What you build and prove:**
...
<details>
<summary><b>Context: the app already exists centrally (read once)</b></summary>

- The VantageDesk API already runs in the shared central deployment at https://desk.taskvantage.oktademo.app.
- You build only the Okta side. Your org's new auth server is trusted automatically via JWKS, with no app-side registration or redeploy.
</details>

<details>
<summary><b>Context: the scenario (read once)</b></summary>

- It is later in 2026. The IT help desk wants what Sales got: an agent that looks up tickets, files incidents, and surfaces knowledge base articles for a technician, bounded to what that technician can do.
- Start from the same agent you already registered. No new agent is needed: agents can hold multiple managed connections. You add VantageDesk to its repertoire.
- You put the whole chain through its paces with Kim Liu, who already has the IT help desk role.
</details>
...
- **Virtual Desktop / Lab Toolkit** for invoking tools, tracing the protocol, and reading the central app's access log.
...
<details>
<summary><b>Context: how the two hops fit together (read once)</b></summary>

- The agent presents the step-2 access token to the MCP server, which forwards to VantageDesk. VantageDesk sees a request carrying the user's identity, not the agent's. Audit logs, row-level filtering, and every other user-context behavior work as if Kim had clicked the buttons herself.
...
- **Tokens are scoped per resource, not portable.** The step-2 access token is valid only for **vantage-desk-as**. The same ID-JAG can be exchanged again at **vantage-crm-as** for a different, differently-scoped token. The user's identity is preserved; the capability is per-resource.
</details>
...
**What just changed:** you now hold the four-part template you will reproduce for VantageDesk in the next sections.
...
**What just changed:** vantage-desk-as exists with status **Active**, no scopes and no access policies yet. You add both next.
...
**What just changed:** the **Scopes** tab shows five scopes. These gate every ITSM tool the adapter exposes.
...
- The access policy is the **"what the _user_ may do"** term of the access intersection: where Okta decides which scopes each person can borrow at VantageDesk.
- It differentiates Kim (full ITSM access) from Alex (no ITSM access) when the adapter asks Okta "what can this user do at vantage-desk-as?"
- Building it by hand is setting up the rooms of the agent's second desk before you hand over any keys.
</details>
...
**What just changed:** vantage-desk-as now issues the full ITSM scope set to IT Help Desk and nothing to everyone else.
...
- This is the **"what the agent may do"** term: the agent's own grant, the second key on its ring.
- The new hire now holds a badge for one more desk, but only the scopes you explicitly allow here.
</details>
...
*WARNING: Do **not** choose "Allow all" here. As the Lab 2.9 note explains, "Allow all" makes the adapter request a generic mcp:read scope this auth server doesn't define, and the XAA exchange fails.*

**What just changed:** the agent now shows two managed connections, vantage-crm-as (from Lab 2) and vantage-desk-as (just added). Its reach has doubled.
...
- The Okta side now knows your agent may reach VantageDesk, but your **MCP Adapter** doesn't yet. It caches managed connections and routes each backend audience through its own **resource**.
- In Lab 2.10 the setup helper wired your CRM resource; now you do the same by hand for VantageDesk. This is the step that makes the ITSM tools appear in 4.8.
- Doing the wiring the Lab 2.10 helper had done for CRM is what gives the agent its second desk yourself: the resource is the live bridge between the agent's grant and the **"what the resource exposes"** term.
</details>
...
**What just changed:** the catalog grew from the six CRM tools to twelve, and Kim, a member of both groups, is authorized for all twelve.
...
<details>
<summary>How authorization decides this</summary>

- Before you built VantageDesk, the catalog held only the six CRM tools; there was no Desk resource to expose. Wiring vantage-desk-as and its resource in 4.3 through 4.7 added the six ITSM tools to the catalog **for everyone**.
- Authorization is binary today: a group member uses that resource's full tool set; a non-member sees the same tools but Okta won't authorize them. A graduated manager-vs-rep tool subset is a future model (see lab-infra/README.md).
- Data-level differences still apply: VantageCRM row-filters Kim's CRM results to what her role can see.
</details>
...
- This is the intersection resolving to a single real call: the agent acted, but **as Kim**, bounded by what Kim may do.
- The action is attributable to both, which agent on whose authority, exactly the property the API-key model can never give you.
</details>
...
Every invocation is first-class about *what Okta issued*: the decoded adapter token, the raw HTTP with Okta's correlation id, and the two-hop XAA trace all print before the call is made, then the tool result and the System Log link.
...
- The two-hop exchange carries *both* identities to the app at once: the user as the assertion subject, the agent as the managed connection performing Hop 2. The call lands "as the user" yet stays attributable to both.
- That dual carriage is the **act** chain from the intro, "which agent, on whose authority," made concrete in the token the adapter mints, hop by hop.
</details>
...
The **XAA exchange trace** block lays out the two hops with the live token endpoints and the observable claims:
...
**What just changed:** the log shows the request hit VantageDesk as Kim (Bearer subject kim.liu@atko.email), scoped to itsm.tickets.read, with the agent named as Client for attribution. The actor is Kim; the agent's involvement is a fact, not a substitute for the user.

<details>
<summary>How the Client name resolves</summary>

The Client field reads **TaskVantage Sales Agent**, not a raw client ID: the central app resolves the token's cid to a display name via **AGENT_CID_NAME_MAP**.
</details>
