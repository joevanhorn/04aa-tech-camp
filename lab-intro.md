...
**Same agent, different access.** Throughout the camp, you keep running the same Lab Toolkit actions — **List the agent's tools** or **Invoke a tool** — against different users. The agent's identity does not change. Neither does its configuration between most runs. The catalog every user sees does not change either: tool *visibility* is a property of the agent, so Alex, Susan, Kim, and Frank all see the same tools. Every persona action prints the **decoded token Okta actually issued** for that user (or Okta's verbatim refusal), so what you're watching is the real artifact, not a label the toolkit computed.
