...
The two business apps — **VantageCRM** and **VantageDesk** — are **one central, multi-tenant, API-only deployment** shared by every attendee (resource servers only, no app UI or human sign-in; each app keys the tenant off the access token's issuer). Each attendee gets their own **agent, Okta MCP Adapter, and Okta org**; the **MCP server is one central, shared service**. See `build-specs/adr/0001-central-multitenant-api-only.md` and `build-specs/adr/0002-central-shared-mcp-server.md` for the hosting decisions.
...
| `module-2-bring-agent-under-management.md` | 45 min. Register your pre-installed OpenCode agent, owner + key, managed connection to VantageCRM. |
...
- **Infrastructure-team dependencies.** Build specs for the lab environment that the docs reference: the central, multi-tenant, API-only VantageCRM and VantageDesk apps (tenant-by-issuer), the central MCP server with the 12-tool catalog, the per-attendee Okta MCP Adapter wired for XAA + scope filtering, the **Lab Toolkit** — the single desktop menu utility on the VDI that fronts the environment check, the persona-scoped CRM/Desk reads, the agent tool listing/invocation with always-on decoded-token panels and inline System Log correlation, the access log, CRM resource setup, and the two proof modes (side-by-side allow vs deny; prove-it-can't-be-faked). **The org-side environment automation now lives in [`lab-infra/`](lab-infra/)** — `provision_lab_org.py` (groups/personas/`vantage-crm-as`+policy), `enroll_tenant.py`, `setup-crm-resource.sh` + `wire_adapter_resource.py` (pre-wire CRM end-to-end), and `reset_lab.py` (between-run teardown), all validated live; see `lab-infra/README.md`. **Adapter/infra wiring requirements** (the agent must be marked **DCR-selectable**; the adapter must hold the agent's **active** signing key; the MCP server needs DNS-rebinding protection disabled behind the ALB; the bridge's Okta-management app needs an admin **role**, not just `okta.aiAgents.manage` scope) are captured in `reference/xaa-bridge-wiring-lessons.md`, which records the full known-good bridge configuration and every failure mode found validating it end to end.
