...
[SCREENSHOT: The CRM Read - Cross-Functional group page in Directory > Groups, showing it has no members]
...
[SCREENSHOT: The vantage-crm-as access policy showing rule 4 (Cross-functional readers) gating the CRM Read - Cross-Functional group with the full crm.* scope set]
...
[SCREENSHOT: The Access Requests catalog entry for CRM Read - Cross-Functional, showing the 2-hour default duration and manager approval]
...
2. Go to your Okta End-User Dashboard at `https://demo-aqua-hoverfly-22341.okta.com` and sign in as Frank (`frank.boone@atko.email` / `Tra!nme4321`).
...
[SCREENSHOT: Frank's End-User Dashboard access request for CRM Read - Cross-Functional with the justification entered and duration set to 2 hours, ready to submit]
7. Click **Submit**.

Frank now sees the request status as **Pending Approval**. Close the incognito window.

[SCREENSHOT: Frank's request showing status Pending Approval on his End-User Dashboard]
...
[SCREENSHOT: The pending approval detail in Access Requests > Pending my approval — Frank's requester, requested access, justification, and 2-hour duration]
4. Click **Approve**.
...
[SCREENSHOT: Frank's access request now showing status Approved]
...
[SCREENSHOT: The CRM Read - Cross-Functional group now showing Frank Boone as a member with an expiry time]
...
== The agent's tools - and what Okta lets Frank Boone use ==
   ...
   With Frank Boone's entitlements, Okta authorizes 6 of 6:
     [USABLE]  aus15g048hlwQzjYZ698__crm.lookup_account
     [USABLE]  aus15g048hlwQzjYZ698__crm.create_account
     [USABLE]  aus15g048hlwQzjYZ698__crm.update_account
     [USABLE]  aus15g048hlwQzjYZ698__crm.lookup_contact
     [USABLE]  aus15g048hlwQzjYZ698__crm.lookup_opportunity
     [USABLE]  aus15g048hlwQzjYZ698__crm.update_opportunity
...
[SCREENSHOT: The Lab Toolkit tool listing for Frank Boone showing all 6 CRM tools as USABLE]
...
[SCREENSHOT: The certification campaign wizard configured — name, group CRM Read - Cross-Functional, reviewer Resource owner, On revoke Remove access]

The resource type choices are **Application**, **Group**, **AI Agent** (plus **Collection** if that EA feature is enabled). Picking the group scopes the campaign to its active memberships — exactly the people granted access through the request flow, Frank included. You are the resource owner, so the review lands in your queue. **On revoke → Remove access** is what makes a **Revoke** decision actually pull the user out of the group; **No response** governs reviews left unanswered past the deadline.

The campaign lands in **Scheduled** state and becomes **Active** at the start time you set.

[SCREENSHOT: The new certification campaign listed in Scheduled state]
...
[SCREENSHOT: The active campaign showing Frank's membership with a Revoke decision and the reason entered]
5. Click **Confirm**.
...
== The agent's tools - and what Okta lets Frank Boone use ==
   ...
   With Frank Boone's entitlements, Okta authorizes 0 of 6:
     [BLOCKED] aus15g048hlwQzjYZ698__crm.lookup_account
     [BLOCKED] aus15g048hlwQzjYZ698__crm.create_account
     [BLOCKED] aus15g048hlwQzjYZ698__crm.update_account
     [BLOCKED] aus15g048hlwQzjYZ698__crm.lookup_contact
     [BLOCKED] aus15g048hlwQzjYZ698__crm.lookup_opportunity
     [BLOCKED] aus15g048hlwQzjYZ698__crm.update_opportunity
...
[SCREENSHOT: The Lab Toolkit tool listing for Frank Boone now showing all 6 CRM tools as BLOCKED]
...
*NOTE — revocation timing: the membership change hits the directory immediately, but Okta keeps honoring Frank's current access token until it's re-minted (up to ~1 hour). So right after the revoke this step may still show all six USABLE — the revoke is enforced at the next token exchange. To enforce it now: end the user's sessions with **Universal Logout** (the "User session" tab, see 5.8), or deactivate the agent (5.8).*
...
[SCREENSHOT: The Deactivate confirmation dialog for the TaskVantage Sales Agent]

The agent's status changes from **ACTIVE** to **DEACTIVATED**. Deactivating immediately kills every tool call through the agent — the adapter returns 401 on any further request. That makes deactivation a true, instant kill switch.

[SCREENSHOT: The TaskVantage Sales Agent page showing status DEACTIVATED]
...
[SCREENSHOT: The Lab Toolkit invoke result as Kim Liu returning 401 Unauthorized because the agent is DEACTIVATED]
...
[SCREENSHOT: The TaskVantage Sales Agent page showing status back to ACTIVE]
...
[SCREENSHOT: The Lab Toolkit invoke result as Kim Liu returning the successful TKT-1734 lookup after reactivation]
