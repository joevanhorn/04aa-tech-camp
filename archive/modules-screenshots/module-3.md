<!-- READINESS: 6 markers. Surfaces: admin-console ×2 (3.2 access-policy rules, 3.7 System Log token-exchange events) — admin-login-only, headlessly capturable. lab-toolkit/VDI ×4 (3.4 Alex USABLE, 3.5 Susan USABLE, 3.6 Frank BLOCKED list, 3.6 Frank invoke denial) — need the VDI Lab Toolkit + reachable MCP/bridge + persona tokens. -->
...
[SCREENSHOT: The vantage-crm-as Access Policies tab showing the four group-to-scope rules (Sales managers, Sales reps, IT help desk, Cross-functional readers) and the catch-all Deny rule]
...
[SCREENSHOT: The Lab Toolkit "List the agent's tools" output for Alex Martinez showing all 6 CRM tools as USABLE]
...
[SCREENSHOT: The Lab Toolkit "List the agent's tools" output for Susan Potter showing all 6 CRM tools as USABLE]
...
[SCREENSHOT: The Lab Toolkit "List the agent's tools" output for Frank Boone showing all 6 CRM tools as BLOCKED]
...
[SCREENSHOT: The Lab Toolkit "Invoke a tool" result for Frank Boone invoking crm.lookup_account, showing the BLOCKED / Authentication failed for resource denial]
...
[SCREENSHOT: The System Log filtered on target.type eq "AIAgent" showing the token-exchange event chain (request, scope_evaluation, grant/deny) across the three tool-listing runs]
