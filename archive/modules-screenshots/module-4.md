<!-- READINESS: 9 markers. Surfaces: admin-console ×4 (4.3 new vantage-desk-as, 4.4 Scopes tab, 4.5 access policy + rules, 4.6 two managed connections) — admin-login-only, headlessly capturable, but these are BUILD steps so the org must be at the pre-Module-4 state for a clean capture. adapter-admin-UI ×1 (4.7 two resources after sync). lab-toolkit/VDI ×4 (4.8 Kim 12 USABLE, 4.9 invoke TKT-1734, 4.10 verbose XAA token dump, 4.11 access-log line) — need VDI Lab Toolkit + reachable MCP/bridge + built vantage-desk-as. -->
...
[SCREENSHOT: The newly created vantage-desk-as authorization server Settings tab showing status Active and audience api://vantage-desk, alongside vantage-crm-as in the Security > API list]
...
[SCREENSHOT: The vantage-desk-as Scopes tab showing the five itsm.* scopes (itsm.tickets.read, itsm.tickets.write, itsm.incidents.read, itsm.incidents.write, itsm.kb.read)]
...
[SCREENSHOT: The vantage-desk-as Access Policies tab showing the "TaskVantage Sales Agent — Desk policy" assigned to the agent client, with Rule 1 (IT help desk — full access) and Rule 2 (Catch-all deny)]
...
[SCREENSHOT: The TaskVantage Sales Agent Managed connections tab now showing two connections — vantage-crm-as and vantage-desk-as (Only allow with the five itsm.* scopes)]
...
[SCREENSHOT: The adapter admin console Resources view for TaskVantage Sales Agent now showing two resources — the CRM resource at /crm/mcp and the enabled Desk resource at mcp-desk.taskvantage.oktademo.app/mcp, auth okta-cross-app]
...
[SCREENSHOT: The Lab Toolkit "List the agent's tools" output for Kim Liu showing all 12 tools USABLE — the 6 crm.* tools plus the 6 newly-appearing itsm.* tools namespaced under the desk-as id]
...
[SCREENSHOT: The Lab Toolkit "Invoke a tool" result for Kim Liu calling itsm.lookup_ticket on TKT-1734, showing the XAA exchange lines and the returned ticket details]
...
[SCREENSHOT: The Lab Toolkit verbose XAA output showing the decoded token chain — user subject token, agent client assertion, step-1 token-exchange POST, ID-JAG, step-2 jwt-bearer POST, issued access token, and the final Bearer tool invocation]
...
[SCREENSHOT: The Lab Toolkit "Show the access log" output with the TKT-1734 line showing Bearer subject kim.liu@atko.email, Client TaskVantage Sales Agent, Audience api://vantage-desk, Scopes itsm.tickets.read]
